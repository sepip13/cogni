...
export async function POST(req: NextRequest) {
  ...
  let body: { modelId?: string; label?: string; desc?: string; provider?: string; tier?: string };
  try {
    body = await req.json();
  } catch {
...
export async function PATCH(req: NextRequest) {
  ...
  let body: { id?: string; label?: string; desc?: string; provider?: string; tier?: string; isActive?: boolean; sortOrder?: number };
  try {
    body = await req.json();
  } catch {
    return NextResponse.json({ error: "Invalid JSON" }, { status: 400 });
