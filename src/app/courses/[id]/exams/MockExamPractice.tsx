...
export function MockExamPractice({
  courseId,
  mockId,
  questions,
}: {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/courses/${courseId}/exams/mock/${mockId}/grade`, {
        method: "POST",
  ...
  return (
    <div style={{ background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: 12, padding: 18, marginTop: 12 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
        <div style={{ flex: 1, height: 5, background: "var(--surface)", borderRadius: 3, overflow: "hidden" }}>
          <div style={{ height: "100%", width: `${((current + 1) / questions.length) * 100}%`, background: "linear-gradient(90deg, var(--accent), var(--accent-2))" }} />
        </div>
        <span style={{ fontSize: 12, color: "var(--text-dim)", fontFamily: "var(--font-jetbrains), monospace", flexShrink: 0 }}>{current + 1} / {questions.length}</span>
      </div>
